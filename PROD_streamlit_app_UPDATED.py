...
def display_top_action_items(page_key, project_filter=None, office_name_filter=None, program_manager_filter=None, project_manager_filter=None, project_stage_filter=None):
    ...
    if project_manager_filter and project_manager_filter != 'All Project Managers':
        if 'project_manager' in procore_df.columns:
            procore_df = procore_df[procore_df['project_manager'].astype(str) == str(project_manager_filter)]
...
def load_data():
    ...
    try:
        ...
        if not summary_df.empty:
            ...
            summary_df = summary_df[summary_df['impact_category'] == 'cost']
            # Apply VISTA filter - exclude projects with VISTA_filter = 1
            if 'VISTA_filter' in summary_df.columns:
                summary_df = summary_df[summary_df['VISTA_filter'] == 0]
            data['executive_summary'] = summary_df
        else:
        ...
        for phase in ['estimating', 'preconstruction', 'construction', 'closeout']:
            ...
            try:
                ...
                if not df_p.empty and not df_k.empty:
                    ...
                    # Apply VISTA filter - exclude projects with VISTA_filter = 1
                    if 'VISTA_filter' in df_p.columns:
                        df_p = df_p[df_p['VISTA_filter'] == 0]
                    if 'VISTA_filter' in df_k.columns:
                        df_k = df_k[df_k['VISTA_filter'] == 0]
...
def display_executive_summary(data, summary_for_impact_calc, impact_category_filter, filters):
    ...
    with col_actions:
        ...
        display_top_action_items("executive_summary", filters['project'], filters['office_name'], filters['program_manager'], filters['project_manager'], filters['project_stage'])
...
def display_phase_summary_page(phase_key, data, impact_category_filter, filters):
    ...
    with col_actions:
        ...
        # --- NEW: Display the new Action Items section ---
        display_top_action_items(phase_key, filters['project'], filters['office_name'], filters['program_manager'], filters['project_manager'], filters['project_stage'])
