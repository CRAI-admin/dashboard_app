...
try:
    ...
    health_check = st.query_params.get("health")
except AttributeError:
    # Fallback for older Streamlit versions
    query_params = st.experimental_get_query_params()
    health_check = query_params.get("health", [None])[0]
...
# --- S3 Helper Functions ---
@st.cache_data(ttl=60)  # Cache for 1 minute to reduce S3 calls
...
def create_bordered_container():
    """Create a bordered container that works across Streamlit versions."""
    try:
        # Try the new syntax first (Streamlit 1.28.0+)
        return st.container(border=True)
    except TypeError:
        # Fallback for older versions - use expander as a container
        return st.container()
...
def calculate_cost_improvement(phase_score, phase_key):
    ...
    # Special formula for preconstruction phase
    if phase_key == 'preconstruction':
        return 0.0129 * ((100 * phase_score) ** 0.4875)
    
    # Special formula for estimating phase
    if phase_key == 'estimating':
        return 0.0086 * ((100 * phase_score) ** 0.4875)
    
    # Special formula for closeout phase
    if phase_key == 'closeout':
        return 0.0054 * ((100 * phase_score) ** 0.4875)
    
    # Fallback for any unknown phases
    return phase_score * 0.03
...
def calculate_executive_cost_improvement(summary_df):
    ...
    estimating_improvement = 0.0086 * ((estimating_score) ** 0.4875) * 100  # Same as estimating page
    preconstruction_improvement = 0.0129 * ((preconstruction_score) ** 0.4875) * 100  # Same as preconstruction page
    construction_improvement = 0.0228 * ((construction_score) ** 0.4875) * 100  # Same as construction page
    closeout_improvement = 0.0054 * ((closeout_score) ** 0.4875) * 100  # Same as closeout page
...
def display_kpi_table(kpi_df):
    ...
    with create_bordered_container():
...
def display_top_action_items(page_key, project_filter=None, office_name_filter=None, program_manager_filter=None, project_stage_filter=None, project_manager_filter=None, vista_filter=None):
    ...
    # Apply project stage filter if specified
    if project_stage_filter and project_stage_filter != 'All Project Stages':
        if 'project_stage' in procore_df.columns:
            procore_df = procore_df[procore_df['project_stage'].astype(str) == str(project_stage_filter)]

    # Apply project manager filter if specified  
    if project_manager_filter and project_manager_filter != 'All Project Managers':
        if 'project_managers' in procore_df.columns:
            procore_df = filter_by_project_manager(procore_df, project_manager_filter)

    # Apply vista filter if specified
    if vista_filter and vista_filter != 'All Projects':
        if 'vista_filter' in procore_df.columns:
            if vista_filter == 'name does not include "VISTA"':
                procore_df = procore_df[procore_df['vista_filter'] == 0]
            elif vista_filter == 'name does include "VISTA"':
                procore_df = procore_df[procore_df['vista_filter'] == 1]
...
def filter_data_efficiently(original_data, selected_project, selected_office_name, selected_program_manager, selected_project_manager, selected_project_stage, selected_impact_category, selected_vista_filter):
    ...
    try:
        ...
        if selected_vista_filter != 'All Projects':
            if selected_vista_filter == 'name does not include "VISTA"':
                summary_df = summary_df[summary_df['vista_filter'] == 0]
            elif selected_vista_filter == 'name does include "VISTA"':
                summary_df = summary_df[summary_df['vista_filter'] == 1]
...
def display_executive_summary(data, summary_for_impact_calc, impact_category_filter, filters):
    ...
    with col_actions:
        ...
        display_top_action_items("executive_summary", filters['project'], filters['office_name'], filters['program_manager'], filters['project_stage'], filters['project_manager'], filters['vista_filter'])
...
def display_phase_summary_page(phase_key, data, impact_category_filter, filters):
    ...
    with col2:
        ...
        if not config_for_phase:
            ...
            for category, config in config_for_phase.items():
                ...
                # Apply special formula for preconstruction phase cost improvement
                elif phase_key == "preconstruction" and category == "cost":
                    percentage_value = 0.0129 * ((phase_score) ** 0.4875) * 100
                # Apply special formula for estimating phase cost improvement
                elif phase_key == "estimating" and category == "cost":
                    percentage_value = 0.0086 * ((phase_score) ** 0.4875) * 100
                # Apply special formula for closeout phase cost improvement
                elif phase_key == "closeout" and category == "cost":
                    percentage_value = 0.0054 * ((phase_score) ** 0.4875) * 100
                else:
    ...
    with col_actions:
        ...
        # --- NEW: Display the new Action Items section ---
        display_top_action_items(phase_key, filters['project'], filters['office_name'], filters['program_manager'], filters['project_stage'], filters['project_manager'], filters['vista_filter'])
...
def main():
    ...
    try:
        ...
        filters = {
            ...
            "vista_filter": st.sidebar.selectbox("Vista", ['All Projects', 'name does not include "VISTA"', 'name does include "VISTA"'])
        }
        ...
        if filters['vista_filter'] != 'All Projects':
            if filters['vista_filter'] == 'name does not include "VISTA"':
                summary_for_impact_calc = summary_for_impact_calc[summary_for_impact_calc['vista_filter'] == 0]
            elif filters['vista_filter'] == 'name does include "VISTA"':
                summary_for_impact_calc = summary_for_impact_calc[summary_for_impact_calc['vista_filter'] == 1]
